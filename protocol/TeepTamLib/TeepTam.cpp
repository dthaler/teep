...
int StartTamBroker(const char* manifestDirectory, int simulate_tee)
{
    int result;

#ifdef TEEP_USE_TEE
    result = StartTamTABroker(manifestDirectory, simulate_tee);
    if (result)
        return result;
#endif

    result = TeepInitialize();
    if (result != 0) {
        return result;
    }
    ...
    result = ConfigureManifests(path, TRUE);
    sprintf_s(path, sizeof(path), "%s%s", manifestDirectory, "/optional");
    result = ConfigureManifests(path, FALSE);
